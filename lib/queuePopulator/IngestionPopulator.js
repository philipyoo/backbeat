...
class IngestionPopulator {
    ...
    /**
     * Close all provision dispatchers
     * @param {function} cb - callback function
     * @return {undefined}
     */
    close(cb) {
    ...
    // specifically for ingestion - dynamically add new log sources
    addNewLogSource(newSource, cb) {
        this._extension.createZkPath(err => {
            ...
            const zookeeperUrlIngest =
                ...
                `${newSource.zenkoBucketName}`;
            const zkEndpointIngest = `${zookeeperUrlIngest}/raft-id-dispatcher`;
            return this._subscribeToLogSourceDispatcher(zkEndpointIngest,
                newSource, this.extConfigs.ingestion.zookeeperPath, cb);
        }, newSource);
    }

    _subscribeToLogSourceDispatcher(zkEndpoint, bucketd, ingestionPath, cb) {
        ...
        raftIdDispatcher.subscribe((err, items) => {
            ...
            this._provisionedIngestionSources[bucketd.zenkoBucketName] =
                raftIdDispatcher;
            ...
            const newRaftReaders = items.map(
                token => {
                    // TODO-FIX: will token ever be 'mongo'?
                    if (token === 'mongo') {
    ...
    _setupIngestionExtension(cb) {
        // dealing with a single extension. Init sources by
        // calling 'addNewLogSource'
        return this._extension.setupZookeeper(cb);
    }

    // TODO: original code overwrote `this.logReaders` whenever a new update
    // was made. Why?
    ...
    _setupUpdatedReaders(done) {
        ...
        // TODO-FIX: may require some sort of locking here
        this.logReadersUpdate = [];
