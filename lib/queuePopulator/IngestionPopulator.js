...
const { MongoMDWrapper } = require('../../extensions/ingestion/utils');
...
class IngestionPopulator {
    ...
    open(cb) {
        ...
        async.series([
            ...
            next => {
                this._mongoClient = new MongoMDWrapper(this.qpConfig.mongo);
            ...
            this._locationDetails = config.getCachedLocationDetails();
            config.on('location-detail-update', () => {
                this._locationDetails = config.getCachedLocationDetails();
            });
            return cb();
        });
    ...
    // check mongo for new buckets, apply any updates
    applyUpdates(cb) {
        this._mongoClient.getIngestionBuckets((err, buckets) => {
            ...
            // [
            //     {
            //         "name": "ace1",  // bucket name
            //         // bucket ingestion status
            //         "ingestion": { Status: 'Enabled' },
            //         "locationConstraint": "myring"
            //     },
            // ]
            ...
            // {
            //     myring: {
            //         accessKey: '30DULZG5ALDRD9VFQZET',
            //         bootstrapList: [],
            //         bucketName: 'bucket1',
            //         endpoint: 'http://10.100.1.128:8000',
            //         secretKey: 'TX3MXSGl85QNcWytS0rY26O'...,
            //         locationType: 'ring'
            //     }
            // }
            ...
            // previous ingestion sources to validate
            const previousSources =
                Object.keys(this._provisionedIngestionSources);

            return async.series([
                done => async.each(buckets, (bucket, next) => {
                    const key = `${bucket.locationConstraint}:${bucket.name}`;
                    if (!previousSources.includes(key)) {
                        ...
                        // TODO: this will need to change for non-ring sources
                        const newSource = {
                            name: bucket.name,
                    ...
                    const index = previousSources.findIndex(s => s === key);
                    previousSources.splice(index, 1);
                    return next();
                }, done),
                done => async.each(previousSources,
