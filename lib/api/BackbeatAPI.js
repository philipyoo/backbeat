...
const { getSortedSetKey } = require('../util/sortedSetHelper');
...
class BackbeatAPI {
     ...
     * @return {undefined}
     */
    _getFailedCRRResponse(score, members, sitename, cb) {
        const response = {
        ...
        return async.eachSeries(members, (member, next) => {
    ...
    getFailedCRR(details, cb) {
        ...
        return this._scanAllKeys(pattern, 0, [], (err, score, keys) =>
            this._getFailedCRRResponse(cursor, keys, sitename, cb));
    }
    ...
    getSiteFailedCRR(details, cb) {
        ...
        return this._getMembers(score, keys, (err, nextMarker, members) =>
            this._getFailedCRRResponse(nextMarker, members, sitename, cb));
    }

    _getMembers(score, keys, cb) {
        ...
        return async.doWhilst(next => {
            const key = keys[i++];
            async.waterfall([
                done => this._redisClient.zcard(key, done), (count, done) => {
                    let start = 0;
                    const stop = -1;
                    // TODO: Check this logic.
                    const totalMembersAvailable = count + members.length;
                    // TODO: Check this logic.
                    if (totalMembersAvailable > listingLimit) {
                        start = count - (listingLimit + 1)
                    }
                    return this._redisClient.zrange(key, start, stop, done);
                },
                (results, done) => {
                    // TODO: Does this correctly maintain the listing order?
                    members.push(...results);
                    if (members.length > listingLimit) {
                        const member = members[listingLimit + 1];
                        return this._redisClient.zscore(key, member,
                            (err, score) => {
                                ...
                                nextMarker = score;
                                members = members.slice(0, listingLimit);
                                return done();
                            });
                    }
                    return done();
        ...
        () => (i < keys.length && members.length < listingLimit),
        err => cb(err, nextMarker, members));
    }
