...
// const { } = require('../util/getFailedCRRKey');
...
class BackbeatAPI {
    ...
    _getFailedCRRResponse(score, keys, cb) {
        ...
        // TODO: Do I need to filter the sorted set keys?
    ...
    getSiteFailedCRR(details, cb) {
        ...
        const nextMarker = Number.parseInt(marker, 10) || 0;
        const timestamps = this._statsClient.getSortedSetHours(Date.now());
        const keys =
            timestamps.map(timestamp => getSortedSetKey(site, timestamp));
        return this._getMembers(nextMarker, keys, (err, score, keys) =>
            this._getFailedCRRResponse(score, keys, cb));
    }
