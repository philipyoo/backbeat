...
class BackbeatAPI {
     ...
     * @param {[type]} bucket - The bucket that the key belongs to
     * @param {[type]} key - The key
     * @param {[type]} versionId - The key's version ID
     * @param {[type]} storageClass - The storage class of the CRR operation
     * @param {Function} cb - The callback to call
     * @return {undefined}
     */
    _deleteProgressKeys(bucket, key, versionId, storageClass, cb) {
        const keys = [
            `${storageClass}:${bucket}:${key}:${versionId}:` +
            `${redisKeys.objectBytes}`,
            `${storageClass}:${bucket}:${key}:${versionId}:`
            `${redisKeys.objectBytesDone}`,
        ];
