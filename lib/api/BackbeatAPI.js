...
class BackbeatAPI {
    ...
    /**
     * Check pause/resume status of sites until condition is met or max retry
     * attempts are made
     ...
     * @return {undefined}
     */
    _retryCheck(sites, condition, cb) {
        // TODO: rename function. Maybe something along lines of diff checker?
        const storage = {};

        /*
            condition = {
                paused: true/false,
                // if false, expect all to be deleted. if true, just check that
                // data we get has a schedule and that it doesn't match with old
                // schedule (if any)
                scheduledResume: true/false,
                // compare all old scheduled times with times from each check
                oldSchedules: [
                    {
                        site1: date,
                    },
        ...
        */
        ...
        // dupe list of sites
        const sitesToCheck = sites.slice();
        // paused or scheduledResume
        ...
        return async.retry({
        ...
        }, done => this._getZkStateNodeDetails(sitesToCheck, (err, data) => {
            if (err) {
            ...
            Object.keys(data).forEach(site => {
                ...
                // update sites to check
                if (type === 'paused' &&
                data[site].paused === condition.paused) {
                    sitesToCheck.splice(sitesToCheck.indexOf(site), 1);
                } else if (type === 'scheduledResume' &&
                condition[type] === false &&
                data[site].scheduledResume === null) {
                    // expect scheduledResume to be cancelled
                    sitesToCheck.splice(sitesToCheck.indexOf(site), 1);
                } else if (type === 'scheduledResume' &&
                condition[type] === true) {
                    // expect scheduledResume to have changed
                    const oldSchedule = condition.oldSchedules.find(s =>
                        Object.keys(s)[0] === site);
                    if (oldSchedule[site] !== data[site].scheduledResume) {
                        sitesToCheck.splice(sitesToCheck.indexOf(site), 1);
                    }
                }
            });
    ...
    pauseService(details, body, cb) {
        ...
        return this._getZkStateNodeDetails(sites, (err, data) => {
            ...
            const isPaused = sites.every(site => data[site].paused);
            if (isPaused) {
                this._logger.info(`${service} service for locations ${sites}` +
                    ' are already paused');
                const statuses = {};
                Object.keys(data).forEach(site => {
            ...
            return this._retryCheck(sites, condition, (err, data) => {
                if (err) {
                    const failedSites = Object.keys(data).filter(site =>
                        !data[site] || !data[site].paused);
                    this._logger.error(`failed to pause ${service} service ` +
                        `for locations: ${failedSites}`, {
                            error: err,
                ...
                this._logger.info(`${service} service paused for ` +
                    `locations: ${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
     ...
     * @return {undefined}
     */
    _getZkStateNodeDetails(service, sites, cb) {
        const stateBySite = {};
        async.each(sites, (site, next) => {
            ...
            if (error) {
                let errMessage = `error getting ${service} state node details`;
    ...
    getResumeCRRSchedule(details, cb) {
        ...
        this._getZkStateNodeDetails(service, sites, (err, data) => {
            if (err) {
                return cb(err);
            }
            const schedules = {};
