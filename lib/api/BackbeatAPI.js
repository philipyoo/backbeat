...
class BackbeatAPI {
    ...
    _deleteProgressKeys(bucket, key, versionId, storageClass, cb) {
        ...
        console.log('_deleteProgressKeys keys:', keys);
        return async.each(keys, (key, next) =>
            this._redisClient.keys(`${key}*`, (err, res) => {
                console.log('_deleteProgressKeys key:', {
                    key: `${key}*`,
                    err,
                    res,
                });
                ...
                return async.eachLimit(res, 10, (resultKey, done) => {
                    console.log({ resultKey });
                    return this._redisClient.del(resultKey, done);
                }, next);
            }),
        cb);
    }
