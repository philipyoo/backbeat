...
class BackbeatAPI {
     ...
     * @return {undefined}
     */
    _checkServiceStateStatus(service, sites, condition, cb) {
        ...
        */
        ...
        }, done => this._getZkStateNodeDetails(service, s, (err, data) => {
            if (err) {
    ...
    pauseService(details, body, cb) {
        ...
        return this._getZkStateNodeDetails(service, sites, (err, data) => {
            ...
            if (alreadyPaused) {
                this._logger.info(`${service} service for location(s): ` +
            ...
            return this._checkServiceStateStatus(service, sites, condition,
            (err, data) => {
                if (err) {
                    ...
                    this._logger.error(`failed to pause ${service} service ` +
                        `for location(s): ${failedSites}`, {
                            error: err,
                            method: 'BackbeatAPI.pauseService',
                        });
                    return cb(err);
                }
            ...
            (err, data) => {
                if (err) {
                    ...
                    this._logger.error('failed to resume replication service ' +
                        `for location(s): ${failedSites}`, {
                            error: err,
                            method: 'BackbeatAPI.resumeService',
                        });
                    return cb(err);
                }

                this._logger.info(`${service} service resumed for ` +
                    `location(s) ${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
