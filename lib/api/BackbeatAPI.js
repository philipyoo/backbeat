...
class BackbeatAPI {
     ...
     * @return {undefined}
     */
    _getEntriesBySite(score, sitename, keys, cb) {
        ...
        let lookUpKeys = keys;
        if (score !== undefined) {
            // TODO: Does this handle edge case where unusual timestamps are given?
            const sortedSetToListFrom =
                this._statsClient.normalizeTimestampByHour(new Date(score));
            const index = keys.findIndex(key => key.includes(sortedSetToListFrom));
            lookUpKeys = lookUpKeys.slice(index, -1);
        }
        // If given score, normalize it and begin the listing from that point in the
        // keys, onward.
        return async.doWhilst(next => {
            ...
            async.waterfall([
                    ...
                    if (score) {
                        return this._redisClient.zrangebyscore(key, score,
                            '+inf', done);
                    }
                ...
                (results, done) => {
                    ...
                    // This will always include the next marker to. i.e. 1001.
                    if (results.length > listingLimit) {
                        const member = results[listingLimit];
                        return this._redisClient.zscore(key, member,
                            (err, score) => {
                                ...
                                nextMarker = score;
                                entries = entries.slice(0, listingLimit);
                                return done();
                            });
                    }
                    return done();
        ...
        () => (i < lookUpKeys.length && entries.length < listingLimit),
