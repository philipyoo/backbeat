...
class BackbeatAPI {
    ...
    _getFailedCRRResponse(cursor, keys, cb) {
        ...
        // const uniqueKeys = [];
        // keys.forEach(key => {
        //     const versionId = key.split(':')[5];
        //     console.log('versionID', versionId);
        //     if (!versionHash[versionId]) {
        //         uniqueKeys.push(key);
        //         versionHash[versionId] = true;
        //     }
        // });
        // return async.eachLimit(uniqueKeys, 10, (key, next) =>
    ...
    _pushToCRRRetryKafkaTopics(queueEntry, cb) {
        ...
        console.log('_pushToCRRRetryKafkaTopics', { queueEntry });
    ...
    _deleteFailedCRRKey(key, cb) {
        const cmd = ['del', key];
        console.log('_deleteFailedCRRKey', { cmd });
    ...
    _decrementFailedMetrics(timestamp, site, contentLength, cb) {
        ...
        console.log('_decrementFailedMetrics', {
            opsFailQuerystring,
            bytesFailQuerystring
        });
        async.parallel([
    ...
    _processFailedKafkaEntries(entries, cb) {
        ...
        return async.eachLimit(entries, 10, (entry, next) =>
            ...
            this._getObjectQueueEntry(entry, (err, queueEntry) => {
                if (err) {
                    // TODO: Do we need to also decrement the failure metrics?
                    // If we cannot retrieve the source object, then we no
                    // longer can consider it a failed operation. Delete the
                    // key used to monitor the failure.
    ...
    retryFailedCRR(details, body, cb) {
        ...
        const uniqueObjs = [];
        reqBody.forEach(obj => {
            const { Bucket, Key, VersionId, StorageClass } = obj;
            const hashKey = `${Bucket}:${Key}:${VersionId}:${StorageClass}`;
            if (!hash[hashKey]) {
                hash[hashKey] = true;
                uniqueObjs.push(obj);
            }
        });
        return async.eachLimit(uniqueObjs, 10, (o, next) => {
            ...
            (err, keys) => {
                ...
                const [key] = keys;
                console.log('retryFailedCRR !', { keys, key });
    ...
    _parseRetryFailedCRR(body) {
        ...
        // TODO: Make sure there are no duplicates in the request.
        if (errMsg) {
            return {
