...
class BackbeatAPI {
    ...
    validateQuery(bbRequest) {
        ...
        if (sitename !== undefined && sitename === '') {
            return errors.InvalidQueryParameter
                .customizeDescription('marker must be a string');
        }
        return null;
    }
    ...
    _findAndDeleteMember(member, cb) {
        // TODO: Search through all the sorted sets and delete the given member.
    }

    /**
     * Builds the failed CRR response.
     * @param {Array} keys - The collection of Redis sorted set keys
     * @param {Function} cb - The callback function
     * @return {undefined}
     */
    _getFailedCRRResponse(score, entries, cb) {
        ...
        return async.eachSeries(entries, (entry, next) => {
            return this._getObjectQueueEntry(entry, (err, queueEntry) => {
                if (err) {
                    // TODO: Understand the logic here. Do we want to skip
                    // these? How will we know which sorted set it's from?
                    //
                    ...
                    return _findAndDeleteMember(member, next);
                }
    ...
    getSiteFailedCRR(details, cb) {
        ...
        const keys =
            timestamps.map(timestamp => getSortedSetKey(sitename, timestamp));
    ...
    _getEntriesAcrossSite(member, cb) {
        ...
        return async.eachLimit(this._validSites, 10, (site, next) => {
            ...
            const keys = timestamps.map(timestamp =>
                getSortedSetKey(site, timestamp));
     ...
     * @param {Number} score - The score to
     * @param {String} sitename
     * @param {Array} keys
     * @param {Function} cb
     * @return {undefined}
     */
    _getEntriesBySite(marker, sitename, keys, cb) {
        // TODO: Need to handle logic for nextMarker.
        ...
        let score = marker;
        if (score) {
            const sortedSetToListFrom =
                this._statsClient.normalizeTimestampByHour(new Date(score));
            const index =
                keys.findIndex(key => key.includes(sortedSetToListFrom));
            lookUpKeys = lookUpKeys.slice(index);
        }
        // If given score, normalize it and begin the listing from that point in
        // the keys, onward.
        return async.doWhilst(next => {
            const key = lookUpKeys[i++];
            async.waterfall([
                done => {
                    // TODO: Check edge cases for intiial score.
                    if (score) {
                        return this._redisClient
                            .zrangebyscore(key, score, '+inf', (err, res) => {
                                if (err) {
                                    return done(err);
                                }
                                score = false;
                ...
                (results, done) => {
                    if (results.length + entries.length >= listingLimit) {
                        const lastIndex = listingLimit - entries.length;
                        const member = results[lastIndex];
                        if (member === undefined) {
                            const slice = results.slice(0, lastIndex);
                            const newEntries = slice.map(member =>
                                (new ObjectFailureEntry(member, sitename)))
                            entries.push(...newEntries);
                            shouldContinue = i < lookUpKeys.length;
                            return done();
                        }
                    ...
                    // TODO: Does this correctly maintain the listing order?
                    shouldContinue =
                        i < lookUpKeys.length &&
                        entries.length < listingLimit;
                    return done();
    ...
    _processFailedKafkaEntries(entries, cb) {
        console.log('_processFailedKafkaEntries', entries);
        const response = [];
    ...
    retryFailedCRR(details, body, cb) {
        ...
        return async.eachLimit(reqBody, 10, (o, next) => {
            ...
            const timestamps = this._statsClient.getSortedSetHours(Date.now());
            const keys = timestamps.map(timestamp =>
                getSortedSetKey(StorageClass, timestamp));
            // TODO: Add locking logic.
            return async.eachLimit(keys, 10, (key, done) => {
                this._redisClient.zrem(key, member, (err, res) => {
                    if (err) {
                        return done(err);
                    }
                    if (res !== 0) {
                        entries.push(new ObjectFailureEntry(member,
                            StorageClass));
                    }
                    return done();
                });
