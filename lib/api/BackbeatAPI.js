...
class BackbeatAPI {
            ...
            this._getObjectQueueEntry(entry, (err, queueEntry) => {
                ...
                return async.series([
                    ...
                    // Delete the Redis lock key
                        ...
                        console.log('DELETING LOCK KEY', lockKey);
    ...
    _normalizeFailMetrics(keys) {
        console.log('_normalizeFailMetrics', keys);
    ...
    _filterFailObjectKeys(keys) {
        ...
        console.log('calling _normalizeFailMetrics with duplicateKeys', duplicateKeys);
        ...
        return async.eachLimit(reqBody, 10, (o, next) => {
            ...
            (err, keys) => {
                ...
                if (keys.length > 1) {
                    ...
                    console.log('calling _normalizeFailMetrics with keys', keys);
                    //  attempt to remove without blocking request
                    const leftoverKeys = keys.slice(1);
                    console.log('calling _normalizeFailMetrics with leftoverKeys', leftoverKeys);
                ...
                return this._redisClient.batch([['get', key]], (err, res) => {
                    ...
                    if (sourceRole) {
                        const lockKey = `${Bucket}:${Key}:${VersionId}:` +
                            `${StorageClass}:processingFailures`;
                        console.log('SETTING LOCK KEY', lockKey);
