...
class BackbeatAPI {
    ...
    _checkServiceStateStatus(sites, condition, cb) {
        // TODO-FIX: need service here
        const storage = {};

        /*
            condition = {
                // expect all sites to match paused state, if given
                paused: true/false,
    ...
    pauseService(details, body, cb) {
        ...
        return this._getZkStateNodeDetails(sites, (err, data) => {
            ...
            if (alreadyPaused) {
                this._logger.info(`${service} service for locations: ` +
                    ` ${sites} are already paused`);
                const statuses = {};
                Object.keys(data).forEach(site => {
    ...
    resumeService(details, body, cb) {
        ...
        return this._getZkStateNodeDetails(service, sites, (err, data) => {
            if (err) {
                return cb(err);
            }
            ...
            (err, data) => {
                ...
                this._logger.info('replication service resumed for locations ' +
                    `${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    statuses[site] = data[site].paused ? 'disabled' : 'enabled';
    ...
    deleteScheduledResumeService(details, cb) {
        ...
        return this._getZkStateNodeDetails(sites, (err, data) => {
            ...
            (err, data) => {
                ...
                this._logger.info('deleted scheduled resume for locations: ' +
                    `${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    statuses[site] = data[site].scheduledResume;
                });
                return cb(null, statuses);
            });
        });
