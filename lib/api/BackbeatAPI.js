...
class BackbeatAPI {
     ...
     * @return {undefined}
     */
    _getFailedCRRResponse(score, keys, cb) {
        console.log('_getFailedCRRResponse', score, keys);
        return undefined;
        const response = {
            // TODO: Update IsTruncated logic.
            IsTruncated: Number.parseInt(score, 10) !== 0,
            Versions: [],
        };
        ...
        return async.eachSeries(keys, (key, next) =>
            this._redisClient.zcard(key, (err, res) => {
                if (err) {
                    return next(err);
                }
                ...
                console.log(res);
                if (res > listingLimit) {
                    const start = res - (listingLimit + 1);
                    const stop = -1;
                    return this._redisClient.zrange(key, start, stop,
                        (err, res) => {
    ...
    getSiteFailedCRR(details, cb) {
        ...
        return this._getMembers(score, keys, (err, nextMarker, members) =>
            this._getFailedCRRResponse(nextMarker, members, cb));
    }
