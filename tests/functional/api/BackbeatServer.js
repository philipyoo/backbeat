...
describe('Backbeat Server', () => {
    ...
    describe('CRR Retry routes', () => {
        ...
        describe.only('Retry feature with Redis', () => {
            ...
            function f(done) {
                ...
                return async.timesLimit(2000, 10, (i, next) => {
                    ...
                    twelveHouseAgo += 5;
                    return addManyMembers(redisClient, 'test-site', members,
                        normalizedScore, twelveHouseAgo, next);
                }, err => {
                    assert.ifError(err);
            ...
            '/_/crr/failed/<bucket>/<key>?versionId=<versionId>', done => {
                ...
                async.series([
                    next =>
                        addMembers(redisClient, 'test-site', [member], next),
                    next =>
                        addMembers(redisClient, 'test-site', [member1], next),
                    next =>
